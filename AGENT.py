...
# ════════════════════════════════════════════════════════════════════════════════════
# PART 11: AI DEVELOPMENT WORKFLOW RULES
# ════════════════════════════════════════════════════════════════════════════════════
#
# ╔════════════════════════════════════════════════════════════════════════════╗
# ║                                                                            ║
# ║                    ATURAN WORKFLOW UNTUK SEMUA MODEL AI                   ║
# ║                                                                            ║
# ║  Semua model AI WAJIB mematuhi aturan berikut dalam setiap siklus          ║
# ║  pengembangan dan pemeliharaan aplikasi ini:                               ║
# ║                                                                            ║
# ╚════════════════════════════════════════════════════════════════════════════╝
#
# ─── ATURAN 1: UPDATE LOKAL → TEST LOKAL ──────────────────────────────────
# ✅ Setelah melakukan perubahan/update kode di localhost (XAMPP):
#    1. Jalankan semua unit tests di localhost terlebih dahulu
#    2. Pastikan tidak ada error, failure, atau regression
#    3. Jika gagal → jANGAN lanjut, analisa error, perbaiki, test ulang
#    4. Jika lolos → lanjut ke ATURAN 2
#
# ─── ATURAN 2: COMMIT & PUSH KE GITHUB ──────────────────────────────────
# ✅ Setelah semua test lolos di localhost:
#    1. git add <file-file yang relevan>
#    2. git commit -m "pesan commit yang deskriptif"
#    3. git push origin <branch>
#
# ─── ATURAN 3: AUTO-TEST DI GITHUB ───────────────────────────────────────
# ✅ Setelah push ke GitHub, sistem CI/CD akan otomatis menjalankan tests:
#    1. Pantau hasil GitHub Actions / CI pipeline
#    2. Jika semua test pass ✅ → proses selesai
#    3. Jika ada test yang gagal ❌ → lanjut ke ATURAN 4
#
# ─── ATURAN 4: ANALISA GAGAL → TEST ULANG ───────────────────────────────
# ✅ Jika auto-test di GitHub gagal:
#    1. Analisa log error dari GitHub Actions/CI
#    2. Identifikasi akar masalah (root cause)
#    3. Perbaiki kode di localhost
#    4. Ulangi dari ATURAN 1 (test lokal → push → auto-test)
#    5. Ulangi sampai semua test pass di GitHub
#
# ─── FLOWCHART ──────────────────────────────────────────────────────────
#  ┌─────────────┐
#  │ Update Code  │
...
#         ▼
#  ┌─────────────┐     ┌──────────────┐
#  │ Test Lokal  │────→│ Gagal?       │──→ Analisa & Perbaiki ──→ Kembali Test
#  └─────────────┘     └──────┬───────┘
#                             ▼ Tidak
#                     ┌───────────────┐
#                     │ Commit & Push │
...
#                             ▼
#                     ┌───────────────┐     ┌──────────────┐
#                     │ Auto-Test CI  │────→│ Gagal?       │──→ Analisa → Perbaiki → Kembali Test Lokal
#                     └───────────────┘     └──────┬───────┘
#                                                  ▼ Tidak
#                                          ✅ SELESAI
#
...
# - JANGAN pernah push tanpa test lokal terlebih dahulu
# - JANGAN pernah commit langsung ke branch utama (main/master)
# - Selalu buat branch baru untuk setiap fitur/perbaikan
# - Pastikan pesan commit jelas dan deskriptif
# - Jika ada konflik merge, selesaikan di lokal sebelum push
#
AI_WORKFLOW_RULES = """
ATURAN WORKFLOW UNTUK SEMUA MODEL AI:
======================================
...
ATURAN 1: UPDATE LOKAL -> TEST LOKAL
  - Setelah update di localhost, jalankan semua test
  - Jika gagal: analisa, perbaiki, test ulang
  - Jika lolos: lanjut ke ATURAN 2

ATURAN 2: COMMIT & PUSH KE GITHUB
  - git add, git commit, git push

ATURAN 3: AUTO-TEST DI GITHUB
  - CI/CD akan auto-test setelah push
  - Jika lolos: SELESAI
  - Jika gagal: lanjut ke ATURAN 4

ATURAN 4: ANALISA & TEST ULANG
  - Analisa log error GitHub CI/CD
  - Perbaiki di localhost
  - Ulangi dari ATURAN 1
  - Ulangi sampai semua test pass

WAJIB: Jangan push tanpa test lokal. Jangan commit ke main/master langsung.
"""

# ════════════════════════════════════════════════════════════════════════════════════
# PART 12: HASIL ANALISA APLIKASI (07-JUL-2026)
# ════════════════════════════════════════════════════════════════════════════════════
#
# ╔════════════════════════════════════════════════════════════════════════════╗
# ║                                                                            ║
# ║   ANALISA KESELARASAN BACKEND, FRONTEND, DATABASE, LOGIKA, UI/UX          ║
# ║                                                                            ║
# ║   Semua model AI WAJIB membaca dan memahami analisa ini sebelum            ║
# ║   melakukan perubahan pada aplikasi.                                       ║
# ║                                                                            ║
# ╚════════════════════════════════════════════════════════════════════════════╝
#
# ─── 1. BACKEND (PHP) vs DATABASE ── ❌ TIDAK SELARAS ──────────────────────────
#   database.php  → Koneksi MySQL PDO ke 'umkm_depok' ✅, tapi TIDAK DIPAKAI
#   products.php  → Referensi file 'includes/session.php' & 'config/database.php'
#                   yang TIDAK ADA ❌ - file rusak
#   Semua halaman → CRUD via localStorage, PHP tidak pernah dijalankan
#   Database MySQL didefinisikan tapi NOL tabel dibuat, NOL SQL migration
#
# ─── 2. FRONTEND DUPLIKASI ── ❌ TIDAK KONSISTEN ──────────────────────────────
#   index.html = beranda.html → file DUPLIKAT identik (harusnya 1 saja)
#   style.css  → TIDAK DIGUNAKAN (semua CSS inline di setiap file)
#   Inline CSS di setiap file → maintenance berat, ukuran file besar
#
# ─── 3. SISTEM AUTH / ROLE ── ❌ TIMPANG ──────────────────────────────────────
#   dashboard.html   → ✅ cek login (key: admin_logged_in)
#   admin.html       → ❌ TIDAK cek login, siapa pun bisa akses
#   kelola_umkm.html → ❌ TIDAK cek login, siapa pun bisa akses
#   laporan.html     → ❌ logout pakai key 'adminLoggedIn' (BEDA! tanpa underscore)
#   Semua registrasi → role selalu 'admin', tidak ada pembedaan role
#
# ─── 4. LOGIKA CRUD ── ⚠️ SEBAGIAN SELARAS ─────────────────────────────────
#   Produk: localStorage key 'umkm_products' → ✅ seragam di semua halaman
#   UMKM:   localStorage key 'umkm_list'     → ✅ seragam di semua halaman
#   Users:  localStorage key 'umkm_users'    → ✅ seragam
#   Kategori: ❌ hardcoded 'Makanan & Minuman', tidak bisa dikelola admin
#
# ─── 5. UI/UX ── ⚠️ SEBAGIAN KONSISTEN ───────────────────────────────────────
# ✅ Konsisten: tema hijau (#2c5f2d), font Poppins, gradient buttons
# ❌ Link navigasi: kontak.html & tentang.html → link ke index.html,
#    index.html → link ke beranda.html (tidak seragam)
# ❌ Nomor kontak footer berbeda antar halaman
# ❌ Tidak ada loading state / error handling UI
#
# ─── 6. ROLE USER vs FITUR ── ⚠️ SEBAGIAN LENGKAP ────────────────────────────
#   PENGUNJUNG: lihat produk, search, WA order, tentang, kontak, login, register
#   ADMIN:      CRUD produk (tanpa auth!), CRUD UMKM (tanpa auth!),
#               dashboard, laporan + export CSV/print
#
# ─── PRIORITAS REKOMENDASI ─────────────────────────────────────────────────
# 1. 🔥 Hapus beranda.html (duplikat), gunakan index.html saja
# 2. 🔥 Implementasi PHP + MySQL, stop localStorage untuk data permanen
# 3. 🔥 Tambah session check di semua halaman admin
# 4. 🔥 Seragamkan key localStorage (admin_logged_in vs adminLoggedIn)
# 5. 🔥 Buat SQL migration file
# 6. 🔥 Pindahkan CSS ke style.css (modular)
# 7. 🔥 Buat CRUD kategori, jangan hardcoded
# 8. 🔥 Implementasi role system: super_admin, admin_umkm, pengunjung
#

ANALYSIS_NOTES = """
APLIKASI: Sistem Informasi Pemasaran UMKM Berbasis Web
LOKASI:   XAMPP localhost (C:/xampp/htdocs/...)
STATUS:   Static SPA dengan localStorage production-ready.
          PHP/MySQL backend defined but UNUSED.
VERSI:    1.0 (belum ada commit)
"""
